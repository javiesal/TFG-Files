...
namespace RosSharp.RosBridgeClient
{
    public class TwistPublisher : UnityPublisher<MessageTypes.Geometry.Twist>
    {
        ...
        //NEW UPDATEMESSAGE
        private void UpdateMessageManually() 
        {
            message.linear = GetGeometryVector3(linearVel.Unity2Ros()); //only Z coord moves linear
            message.angular = GetGeometryVector3(angularVel.Unity2Ros()); //only y coord moves angular
            Publish(message);
        }
