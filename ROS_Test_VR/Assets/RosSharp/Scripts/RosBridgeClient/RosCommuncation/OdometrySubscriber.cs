...
namespace RosSharp.RosBridgeClient
{
    public class OdometrySubscriber : UnitySubscriber<MessageTypes.Nav.Odometry>
    {
        ...
        private void Update()
        {
            ...
            {
                if (position.z > 1000000 || position.z < 1000000)
                    posText.text = "Pos.: \n X: " + position.x.ToString("N2") + " m \n Z: " + "-" + "\n W: " + (2*Mathf.Asin(rotation.y)*180/Mathf.PI ).ToString("N2") + "º";
                else if (position.x > 1000000 || position.x < 1000000)
                    posText.text = "Pos.: \n X: " + "-" + "\n Z: " + position.z.ToString("N2") + " m \n W: " + (2 * Mathf.Asin(rotation.y) * 180 / Mathf.PI).ToString("N2") + "º";
                else
                    posText.text = "Pos.: \n X: " + position.x.ToString("N2") + " m \n Z: " + position.z.ToString("N2") + " m \n W: " + (2 * Mathf.Asin(rotation.y) * 180 / Mathf.PI).ToString("N2") + "º";
